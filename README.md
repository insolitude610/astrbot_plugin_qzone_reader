<div align="center">

<img src="assets/logo.gif" width="150" alt="astrbot_plugin_qzone_reader">

# astrbot_plugin_qzone_reader

把 QQ空间说说转发给 bot，自动读取、总结，并且**内容会进入会话历史**，之后可以继续追问讨论。

不需要任何指令 —— 转发动作本身就是触发条件。

[![AstrBot](https://img.shields.io/badge/AstrBot-%3E%3D4.16%2C%3C5-orange?style=flat-square)](https://github.com/AstrBotDevs/AstrBot)
[![Platform](https://img.shields.io/badge/Platform-aiocqhttp%20%2F%20NapCat-2496ED?style=flat-square)](#已知限制)
[![License](https://img.shields.io/badge/License-MIT-green?style=flat-square)](LICENSE)

</div>

---

## 为什么需要它

QQ空间说说的分享卡片上只有一句标题和摘要，模型看不到正文。**转发（repost）更麻烦**：卡片上能拿到的往往只有转发者自己写的那句短评语，被转发的原文完全不在卡片里。

典型场景：

> 某人转发了一篇瓜条，只写了句「现在的小朋友玩的真花」。
> 卡片上只有这 12 个字，bot 完全不知道那篇瓜条讲了什么。

本插件会把说说（**含被转发的原文**）的完整正文、配图、作者、时间取出来，注入本轮对话。

## 安装

在 AstrBot 的插件目录下：

```bash
cd /AstrBot/data/plugins
git clone https://github.com/insolitude610/astrbot_plugin_qzone_reader
```

也可以在 WebUI 插件页填写仓库地址安装，或上传插件压缩包。

依赖只有 `aiohttp`，AstrBot 本体已自带，**无需额外 pip install**。装好后在 WebUI 重载插件即可。

## 使用

把说说转发给 bot：

1. 在 QQ空间里点说说的「分享」→ 发送给 bot；
2. bot 自动读取并总结；
3. 接着追问即可，比如「作者是什么情绪」「帮我梳理一下时间线」。

也支持直接粘贴链接：`https://h5.qzone.qq.com/ugc/share?...` 或 `https://mobile.qzone.qq.com/l?...`

### 群聊里引用分享卡片

**可以。** 群聊里常见这个用法：

```
群友A：［QQ空间说说分享卡片］
群友B：［引用上面那条］@bot 这瓜啥情况
```

bot 会去读**被引用那张卡片**里的说说，正常总结。原理是 AstrBot 在处理引用消息时会把被引用的消息内容抓回来（默认行为），所以卡片信息拿得到。

几点说明：

- 引用时必须 **@bot 或带唤醒前缀**，否则群聊里 bot 不会被唤醒 —— 这是 AstrBot 的通用规则，跟本插件无关。
- 你**不需要先让 bot 看一遍**原卡片。直接引用 + @它就行，插件会当场解析。
- 引用**纯文本消息**（不是卡片）时不会触发，因为没有可解析的分享链接。
- 嵌套引用（引用一条引用了卡片的回复）也能找到，最多递归 5 层。

### 转发的说说

转发场景下内容会分两层呈现，bot 两层都能看到：

```
转发者：XXX          转发时间：...
转发语：现在的小朋友玩的真花

—— 以下是这条说说转发的原内容 ——
原文作者：YYY        原文发布时间：...
原文正文：（完整原文）
原文配图：共 N 张
```

配图会优先附带**原文的图**，因为转发场景下原文图才是主体。

## 登录态从哪来

QQ空间没有开放接口，读取说说需要账号 Cookie。插件按以下顺序尝试：

1. **NapCat 自动获取**（默认）
   通过 OneBot 的 `get_cookies(domain="user.qzone.qq.com")` 直接向协议端索取，成功后缓存（默认 600 秒，见 `cookie_ttl`）。登录态失效时会自动清缓存重取一次。
2. **手动 Cookie 兜底**
   把 `cookie_source` 设为 `manual`，或在自动获取失败时使用 `cookies_str`。

抓手动 Cookie：电脑浏览器登录 QQ空间 → F12 → Network → 刷新 → 任意请求 → 复制请求头里的完整 `Cookie`，必须包含 `uin`、`skey`、`p_skey`。

> **Cookie 等同于账号密码。** 它只保存在你的 AstrBot 配置里，不会外发，也建议限制 `group_whitelist` 避免群友借用你的登录态。请勿贴到公开群聊或截图分享。

## 配置项

| 配置 | 默认 | 说明 |
| --- | --- | --- |
| `cookie_source` | `"napcat"` | `napcat` 走协议端自动获取；`manual` 只用 `cookies_str` |
| `cookies_str` | `""` | 手动 Cookie，任何模式下都作为兜底 |
| `cookie_ttl` | `600` | 自动获取的 Cookie 缓存秒数，`0` 表示不缓存 |
| `read_full_feed` | `true` | 关闭后只用卡片上的文字，不发任何网络请求 |
| `max_images` | `4` | **最终交给模型的图片总数上限**（切片计入其中）。`0` 表示不带图。无隐藏上限，但张数越多 token 消耗越大 |
| `slice_tall_images` | `true` | 长截图切片，见下文。这是能否读出瓜条配图细节的关键 |
| `slice_max_height` | `0` | 每片目标高度（像素），`0` 用默认 1280。调小更清晰但片数更多 |
| `image_max_width` | `1024` | 图片宽度上限，超出则**等比缩小**（宽高按同比例缩，不会拉变形）。`0` 表示不缩放 |
| `jpeg_quality` | `88` | 切片/缩放后的 JPEG 质量（1-95）。调低明显减体积，代价是小字变糊 |
| `context_mode` | `"keep"` | 上下文保留，见下文 |
| `summarize_mode` | `"brief"` | 总结模式，见下文 |
| `group_whitelist` | `[]` | 只在这些会话生效。填 **AstrBot 的会话 ID（UMO）**，如 `aiocqhttp:GroupMessage:123456789`，从 WebUI「会话管理」页面复制；留空表示全部会话 |
| `notify_on_failure` | `true` | 读不到正文时是否告知用户。关闭则静默降级，只把卡片文字交给模型且不提失败 |

### 上下文保留 `context_mode`

决定说说内容**是否留在会话历史里**：

| 取值 | 行为 | 适合 |
| --- | --- | --- |
| `keep` | 说说原文与配图随本轮消息写入会话历史，后续追问时模型仍能看到内容 | 默认。你希望就这条说说继续聊下去 |
| `once` | 只在**本次回复**时给模型看，不写入历史 | 看一眼就完事的瓜条。避免长瓜条和它的切图长期占着上下文，把后续对话的额度挤掉 |

两者在**当次回复**上的表现完全一样 —— 都是完整读取、照 `summarize_mode` 总结。差别只在**下一轮**：

| | `keep` | `once` |
| --- | --- | --- |
| 你直接追问「第二点展开说」 | ✅ 它记得原文 | ⚠️ 只剩自己的总结，说不出细节 |
| 你**引用那条卡片**再 @bot | ✅ 正常工作 | ✅ 正常工作 |
| 说说内容占用的上下文 | 长期占用 | 不占用 |

### `once` 模式下怎么继续聊

落历史时说说原文被过滤掉，所以第二轮 bot 看到的是：

```
user      |（转发卡片）              ← 卡片本身，不含正文
assistant | 我看完了，这瓜讲的是…    ← 它自己的总结
```

它**记得自己总结过，但已经看不到依据**。直接追问容易得到含糊回答。

正确做法是**引用那条卡片再 @bot**：

```
你：［引用那张分享卡片］@bot 第二点展开说
```

插件会从被引用的消息里重新拿到链接、**重新去 QQ空间抓一遍**，所以照样能答。这条路依赖的是 QQ 服务器上的原消息，**与会话历史无关** —— 也就说 `once` 省的是上下文额度，不是可访问性。

唯一真正失效的情况：**原消息被撤回**，或协议端 `get_msg` 取不到。

> **不会破坏历史结构，但可能导致对话缓存重建。**
> AstrBot 的会话历史是每次请求从数据库读取、写完整体覆盖，`once` 只是让落库的副本少了说说内容；`keep` / `once` 产出的序列都是合法的 user/assistant 交替结构，不会导致上下文错乱或 provider 报错。
> 但由于历史内容发生变化，provider 侧的提示词前缀缓存（如 Gemini 的隐式上下文缓存、Anthropic 的 `cache_control`）**可能失效并重建**，表现为该轮的首 token 延迟与输入计费略有上升，之后恢复正常。

> **另外两点：**
> 1. 你发的那条**卡片消息本身**由 AstrBot 自行入库，本插件无法控制 —— 它仍会留在历史里，只是内容是个空壳卡片，不含说说正文。
> 2. **bot 当次的回复仍然会保存**。`once` 只影响「说说内容」，不影响对话记录本身。

### 长截图切片 `slice_tall_images`

QQ空间的瓜条配图几乎都是**聊天记录长截图**，典型尺寸 `640x3799`、`476x4096`。

这类图直接发送会被视觉模型等比缩放到像素上限（通常约 1024px），**宽度从 640 压到约 170** —— 里面的聊天文字彻底糊掉，模型实际上等于没收到图。

开启切片后，插件会保持宽度不变、只按高度切成多段（段间留重叠避免切断消息）再发送。上面的例子会切成 4 片 `640x1280`，文字保持清晰，模型就能真正读到图里的内容。

代价与约束：

- **片数计入 `max_images`**。一本 14 张长图的瓜条理论能切成 50+ 片，所以最终张数受该上限约束，不会失控。
- 每张图最多切 12 片；高瘦比不足 1.5 倍的正常竖图不会被切。
- token 消耗明显上升。只想让它"看懂文字"的话可以把 `max_images` 设小（如 `4`），或干脆关掉切片。

> 如果你发现 bot 总结瓜条时只说得出"某某开撕某某"这类笼统内容，八成是配图没被有效阅读 —— 这条正是为此而设。

### 请求体积：怎么调小

图片多时（一本瓜条十几张长图能切成几十片）请求体可能到好几 MB。这**不影响功能**（模型照常收到），代价是 token 费用、上传耗时，以及若 `log_level=DEBUG` 时日志会被撑出巨大的单行。

用一组真实数据（12 张长图 → 41 片，原始 6.20 MB）实测各选项的效果：

| 调整 | 体积 | 省 |
| --- | --- | --- |
| 基线 | 6.20 MB | — |
| `image_max_width=1024`（默认已开） | 5.70 MB | -8% |
| `jpeg_quality=75` | 4.25 MB | -31% |
| 两者叠加 | 4.11 MB | -34% |
| `max_images=20` | ~3.03 MB | -51% |
| `max_images=12` | ~1.82 MB | -71% |

**结论：数量是线性杠杆，压缩是边际收益。** 最有效的往往是调小 `max_images` —— 代价是后面的图读不到（额度按顺序先到先得）。

`jpeg_quality` 调低对小字的影响**无法用脚本验证**，建议先改成 75 试一条，确认 bot 还能读出图里的细节再定。

> `image_max_width` 缩的是**宽和高**（等比），不是只截宽度 —— 那会把图压变形。实际效果：
>
> | 原图 | 缩放后 |
> | --- | --- |
> | 1280 × 960 | 1024 × 768 |
> | 1184 × 1280 | 1024 × 1107 |
> | 1024 × 2347 | 不变（等于上限） |
> | 640 × 3799 | 不变（窄于上限） |
>
> 所以瓜条里常见的 640 宽长截图**完全不受影响**，只影响本身就很宽的图。

> **日志层面的独立缓解**：把 AstrBot 的 `log_level` 设为 `INFO`（默认为 INFO），请求体就不会被写进日志。这是与图片体积无关的另一条路。

### 总结模式 `summarize_mode`

这一项决定 bot 拿到说说内容后**怎么回应**，是整套配置里最影响体验的一个：

| 取值 | 行为 | 适合 |
| --- | --- | --- |
| `off` | **不加任何指令**。bot 只知道内容，怎么回完全由它的人格决定，可能只聊一句、也可能问你问题 | 你只想让它"看懂"，具体聊什么由你带节奏。**注意这不代表 bot 不回复**，是否回复由 AstrBot 的唤醒规则决定 |
| `brief` | 附一句「**先简要总结**再说说你的看法」，偏要点式，话不多 | 默认。想要"转发就有一句总结"，又不希望太长 |
| `full` | 明确要求 bot **用自己的人格把这条动态完整总结一遍**：谁说的、发布时间、来龙去脉、涉及哪些人及各自立场、作者情绪、**正文与配图的逐条对应**、以及哪里信息不全 | 长瓜条、需要一次看全的内容。想让它认真梳理而不是随便聊两句 |

`full` 模式下的指令要点（在 `main.py` 的 `FULL_SUMMARY_INSTRUCTION`）：

```
【叙述规范，务必遵守】
A. 全篇用第三人称叙述。总结部分要写「谁主张什么、谁指控什么」，
   可以用「作者称」「他指出」「原文提到」这类转述，
   但不要用第一人称代入任何一方 —— 不要写「我 3 月 6 日进群」这种句子，
   要写成「作者称他 3 月 6 日进群」。只有最外层开场和结尾可以用你自己的口吻。
B. 这是归纳总结，不是把原文缩写一遍。要按条理用自己的话重述要点，
   把散落在原文各处的信息归到对应类别下；不要逐句搬运或改写原句。

1. 谁说给谁听的、发布与转发的时间；
2. 事情的来龙去脉，按条理把要点讲清楚；
3. 涉及哪些人或方，各自的主张与立场；
4. 作者的情绪与意图；
5. 【正文与配图的对应】这是重点，不要只笼统说「配图是聊天记录」。
   正文若用 p1、p3-6 这类编号标注了配图，请沿用原文的编号，
   逐条说明「正文的哪句话 / 第几点」对应「第几张、第几张图」，
   以及那几张图里具体是什么内容（谁和谁的对话、什么截图、能看出什么）。
   正文没有明确编号时，按你看到的图片顺序自行编号说明。
6. 信息不完整或属于单方说法的地方，明确指出来，不要替作者补全。
总结之后可以简短说一句你的看法，但总结本身要完整。
```

**开头的「叙述规范」是两种模式共用的**（`brief` 和 `full` 都带），它是被实际输出教育出来的：

| 规则 | 起因 |
| --- | --- |
| **A. 第三人称** | 早先指令里有「用你平时说话的口吻」，模型理解成「用**说话人**的口吻」，整篇变成作者第一人称自述（「**你说我**『同情做鸡的』根本不是**咱俩**矛盾的重点」「**我** 3 月 6 日进吃瓜群」）。读者分不清哪句是作者说的、哪句是 bot 的判断。现在明确要求转述，并给出反例与正例。 |
| **B. 归纳而非缩写** | 只说「完整地总结」，模型会倾向把原文句子**改写一遍** —— 读起来是浓缩版原文，而不是归纳后的结论。现在明确「不是把原文缩写一遍」。 |

**第 5 条是为「瓜条」这类内容专门加的。** 它们通常正文只写人名和编号（如 `1️⃣某某（p1/2）`），具体事件全在配图里。原来只要求「说明图片内容与正文的关系」，模型往往只回一句「配图是聊天记录截图」就完了。现在要求它**逐条对上编号**，才能读出图里的实际内容。

**第 6 条也是刻意加的**：瓜条类内容往往是单方说法，这条约束能减少 bot 顺着原文情绪跑偏。

### 图片编号是怎么对上的

这里有个容易误解的点：**我发给模型的是切片，不是原图。**

一条 12 张长图的瓜条会被切成 40 多个片段，模型看到的「第 1 个图片块」其实是原图 1 的上半截。所以插件会在注入文本里附一段说明：

```
（说明：长截图会被按高度切成多个片段依次附带，所以附带的图片块数量可能多于
上面的配图张数。片段按顺序排列，相邻片段内容有少量重叠；同一张原图切出的片段
紧挨在一起。正文里用 p1、p2 这类编号引用的「第几张图」，对应的是上面说的配图
张数顺序，不是图片块序号 —— 请按这个对应关系描述。）
```

没有这段说明，模型只会看到一串图片块，无法知道它们对应原文的第几张图，编号就成瞎猜了。

**三种模式的内容都会进入会话历史**，之后可以继续追问，区别只在 bot 第一句回得多完整。

> 旧版配置里的 `auto_summarize`（布尔）仍然兼容：`true` 等价于 `brief`，`false` 等价于 `off`。新键 `summarize_mode` 优先。

## 工作原理

```
转发卡片 / 引用卡片并 @bot / 粘贴链接
      │
      ▼
① 识别链接   递归扫描整条消息链（含被引用的 Reply.chain）
      │        Comp.Json.data · Plain 文本 · raw_message 兜底
      │        并还原 QQ 卡片里的 &#44; 转义
      ▼
② 取登录态   NapCat get_cookies → 手动 Cookie
      │
      ▼
③ 短链解析   跟随 302 跳转到 h5.qzone.qq.com/ugc/share
      │        拿到 res_uin / cellid
      ▼
④ 读分享页   解析页面内嵌的 FrontPage 数据
      │        cell_summary 正文 · cell_pic 配图 · cell_original 被转发的原文
      │        （失败则退回 emotion_cgi_msglist_v6 列表接口）
      ▼
⑤ 配图处理   长截图切片（按 max_images 预算）
      │
      ▼
⑥ 注入       on_llm_request → extra_user_content_parts + image_urls
      │        context_mode=once 时标记为临时内容，不落历史
      ▼
   AstrBot 组装为同一条 user 消息 → 写入会话历史 → 可继续讨论
```

第 ⑥ 步是「能持续讨论」的关键：AstrBot 会把「用户消息 + 额外内容块 + 图片」组装成**同一条 user 消息**并写入会话历史。所以说说内容不是一次性提示，而是真正留在了上下文里。

（`context_mode=once` 时，第 ⑥ 步会把内容标记为临时块，AstrBot 落历史时把它过滤掉 —— 所以这种模式下说说内容不会留在后续上下文里。）

## 排障

先在 AstrBot 日志里搜 `[qzone_reader]`，插件的关键步骤都会打日志。

| 现象 | 原因与处理 |
| --- | --- |
| 日志出现 `没有可用的 QQ空间登录态` | NapCat 未在线或 QQ 已掉线。恢复登录，或手动填 `cookies_str` |
| 日志出现 `放弃读取` | 分享链接里没有可定位说说的参数。把日志里那行链接发出来以便跟进 |
| 日志出现 `分享页没取到内容，改走动态列表接口兜底` | 分享页未返回数据（权限较严的说说）。若列表接口也定位不到，会降级为卡片文字 |
| bot 说读不到内容 | 多为登录态失效，按第一行处理 |
| 总结里没有图片内容 | 模型不支持图片输入时，`context_mode=keep` 下 AstrBot 会把图片换成 `[Image]` 占位并跳过。注意 **`once` 模式下图片走的是额外内容块，AstrBot 的模态过滤管不到它** —— 如果当前模型不支持视觉，请把 `max_images` 设为 `0`，否则可能直接被 provider 拒绝 |
| 日志出现 `非 QQ空间域名，拒绝读取` | 链接的域名不在白名单内（只接受 `https` 的 `*.qzone.qq.com` 与 `qzonestyle.gtimg.cn`）。这是安全门禁，不是故障 |
| 只在某些会话生效 | 用 `group_whitelist` 填**会话 ID（UMO）**，如 `aiocqhttp:GroupMessage:123456789`，从 WebUI「会话管理」页面复制。填群号不生效（口径是 AstrBot 的会话 ID，不是群号） |
| 群聊里 @bot 没反应 | 确认确实 @ 到了 bot（或被唤醒前缀命中）。日志里没有 `检测到 QQ空间分享` 就说明插件没被触发，问题在唤醒环节 |
| 引用卡片后没反应 | 日志里搜 `检测到 QQ空间分享`。没有则说明协议端没取回被引用的原消息（原消息可能已删除），改用直接转发 |
| 不想每次都被回「没读到」 | 把 `notify_on_failure` 设为 `false` |
| 总结瓜条时内容很笼统 | 多半是配图没被有效阅读。确认 `slice_tall_images` 已开启，并把 `max_images` 调大（长截图切片后片数较多，4 片可能只够读 1 张图） |
| 上下文被长瓜条占满 | 调小 `max_images`，或关闭 `slice_tall_images`；也可以把 `context_mode` 设成 `once` 让它不长期占用 |
| `once` 模式下追问得不到细节 | 这是预期行为：原文没进历史。**引用那条卡片再 @bot** 即可，插件会重新抓取 |
| 报了没装 Pillow | 切片依赖 Pillow。AstrBot 本体已依赖它，正常不会缺；确实缺就 `pip install pillow`，插件会自动退回发原图 |

## 已知限制

- 依赖 **aiocqhttp / OneBot v11**（NapCat、LLOneBot 等），其他平台适配器不生效。
- **只接受 `https` 的 QQ空间分享链接**（`*.qzone.qq.com`、`qzonestyle.gtimg.cn`）。这是刻意的安全门禁：插件会带着账号 Cookie 去请求这些地址，所以必须按解析后的 hostname 做白名单，而不是「URL 里出现 qzone.qq.com 就算」。Cookie 也只会发给 `*.qzone.qq.com`、`*.qpic.cn`、`*.qlogo.cn`、`*.gtimg.cn`、`*.photo.store.qq.com` 这些域名。
- 跳转是**逐跳校验**的：如果某个可信地址把你跳到白名单外的域名，那一跳不会带 Cookie，内容也不会被采信。残余风险是「白名单内的域名之间互相跳转仍然带 Cookie」，以及 Referer / Origin 头始终带有你的 QQ 号（配图床需要它，没有一起收紧）。
- 只带 `uin=` 的链接（没有 `res_uin`/`cellid`）在分享页解析失败时**不会被拿去猜**：以前会去那个 uin 的动态列表里按时间戳找，可能读到无关的说说，现在直接放弃。
- `group_whitelist` 比对的是 **AstrBot 的会话 ID（UMO）**，不是群号。若开了 `platform_settings.unique_session`，会话 ID 会带上发送者（`aiocqhttp:GroupMessage:10001_123456789`），按群填的条目会匹配不上 —— 按「会话管理」页面里实际显示的那串填。
- 群聊里**必须 @bot 或带唤醒前缀**才会响应，这是 AstrBot 的通用唤醒规则。没被唤醒时插件**不会做任何抓取**（不抓页面、不下载图片），不会白白消耗资源。
- 引用分享卡片时依赖协议端能取回被引用的原消息（AstrBot 的 `get_reply` 默认开启）。若原消息已被删除或 `get_msg` 失败，引用方式就读不到。
- 只能读取**你的账号有权限看到**的内容；登录态失效时读不到好友可见的说说。
- 分享页对部分权限较严的说说可能不返回数据，此时会退回列表接口；两条路都失败则降级为卡片文字。
- 视频只记录「含视频 N 个」，不解析视频内容（`cell_video()` 已能取地址，但未接入注入）。
- **AstrBot 自身有图片长边上限**（`provider_settings.image_compress_options.max_size`，默认 1280）：符合上限的 JPEG/PNG 会原样透传（所以插件的 `jpeg_quality` 是有效的），但长边超过 1280 的图会被平台按长边重新缩放并重新编码 —— 因此 `slice_max_height` / `image_max_width` 设得比 1280 更大不会生效；另外高度在 1280~1600 之间的长截图低于插件的切片阈值，会走平台的缩放（宽度损失约 2%~20%），而不是切片。
- 你自己在该条消息里附带的图片会排在说说配图**之前**（AstrBot 先放用户图、再放插件注入的图），所以正文里的 `p1/p2` 编号在这种消息里可能对不上，需要模型自己判断。
- 长文本瓜条注入后会占用较多上下文，受模型上下文长度限制。开启长截图切片后图片张数增加，token 消耗更明显 —— 可用 `context_mode=once` 或调小 `max_images` 缓解。
- 长截图切片参数（每片 1280px、高瘦比 1.5 倍判据）是**经验值**，未针对具体模型实测校准。不同视觉模型的上限不同，必要时用 `slice_max_height` 调整。
- 使用的是 QQ空间 Web 端非公开接口，腾讯调整页面结构时可能需要跟进修复。

## 许可

MIT
