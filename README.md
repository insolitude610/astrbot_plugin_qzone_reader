...
不管哪种模式，都是**追加进「本轮那条用户消息」的末尾** —— 和你的话在同一条消息里，不是单独发一条：

```
① user  [你的话] [AstrBot 的 <system_reminder>] [【QQ空间说说原文】+指令] [配图 × N]
② assistant  它的回复
```

（`<system_reminder>` 是 AstrBot 自己加的，插件的内容排在它后面。）

**`keep`**：① 里的原文与配图随消息一起存进会话历史，之后每一轮都带着，所以追问时它仍看得到。

**`once`**：原文与配图只进「当轮请求」，落库时被过滤掉，① 在库里只剩你自己的话 —— 所以是**从来就没写进去**，而不是「被清掉」：

```
第1轮（转发卡片）      发出去 4 块 → 存下来 2 块
① user  [看看这条说说] [reminder]
② assistant 好的，这个说说的内容是…        ← 记得自己总结过，但看不到依据

第2轮（聊别的）                            ← 历史里没有任何说说内容

第3轮（引用卡片再 @bot）  插件当场重新抓一遍，注入到「第3轮这条消息」的末尾
⑤ user  [引用卡片 第二点展开说] [reminder] [说说原文] [图×N]   ← 同样只在当轮有效
⑥ assistant （继续说）

第4轮（没引用，直接追问）                  ← 又看不到了，要再引用一次
```

一句话：`once` 下说说内容只活在「读到它的那一轮」，永远不进历史；想再看就**引用卡片再 @bot**，插件会重新抓一遍（走的是 QQ 服务器上的原消息，与会话历史无关）。
