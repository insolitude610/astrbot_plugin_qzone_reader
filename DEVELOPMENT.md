...
| 常量 | 说明 |
| --- | --- |
| `NARRATION_RULES` | **两种模式共用**的叙述规范：A 第三人称叙述、B 归纳而非缩写。拼在指令最前面 |
| `BRIEF_SUMMARY_INSTRUCTION` | `summarize_mode=brief` 的指令：要点式总结后自然接话 |
| `FULL_SUMMARY_INSTRUCTION` | `summarize_mode=full` 的指令：以完整总结为主体，6 条要点清单，第 5 条要求逐图对应 |
| `FAILURE_HINT` | 读取失败时给模型的提示，明确要求「不要编造」 |

### `NARRATION_RULES` 的由来（两次都是被实际输出教育的）

这两条不是设计出来的，是**看到真实回复有问题才加的**：

**A. 第三人称** —— 早先指令里有一句「用你平时说话的口吻」。模型的解读是「用**说话人**的口吻」，于是整篇变成作者视角的自述：

```
一、对 ymm：你说我「同情做鸡的」根本不是咱俩矛盾的重点。
我 3 月 6 日进吃瓜群，原本了解的是舞萌圈内事……
```

读者根本分不清哪句是作者说的、哪句是 bot 的判断。删掉那句，改为明确要求转述，并给出**反例 + 正例**：

- 反例：`我 3 月 6 日进群`
- 正例：`作者称他 3 月 6 日进群`

**B. 归纳而非缩写** —— 只说「完整地总结」，模型倾向把原文句子改写一遍。对照两次输出可见差别：合格的那次有 20+ 处独立事实标注（人名＋具体行为＋时间），不合格的那次大量是原句改写。所以补上「不是把原文缩写一遍……不要逐句搬运或改写原句」。

改这两个常量时注意：`test_core.py` 的 `[9]` 段有 15 条断言盯着它们，包括「旧措辞已移除」和「两种模式都要带上叙述规范」。`off` 模式**不带**任何指令（这是它的定义）。

`core/qzone_api.py` 里还有一个 `IMAGE_LAYOUT_HINT`：交代切片结构，让模型能把「正文里的 pN」对上「看到的第几个图片块」。**这是逐图对应的前提** —— 切片后图片块数远多于原图张数，不说明的话模型只能瞎猜编号。

`to_prompt()` 里它的出现位置有讲究（见 `[8d]` 段断言）：

- 转发场景：附在「原文配图」说明之后
- 非转发场景：附在「配图」说明之后
- **两者只出现一次**，不重复；`max_images=0`（不带图）时不出现
...
- `test_core.py` 的 `[9]` 段断言了 `full` 必须包含「完整地总结」「来龙去脉」「作者的情绪」「配图」「信息不完整」等关键词，以及**逐图对应**相关措辞（`正文与配图的对应`、`沿用原文的编号`、`p1`、`逐条说明`、`第几张图`、`自行编号`、`不要只笼统说`）。改指令时同步改测试，否则会红。
- 第 5 条里「沿用原文的编号」和「自行编号」两句是**成对**的：瓜条正文常自带 `p1/2`、`p3-6` 这类编号，能沿用就沿用；没有编号时才让模型自己排。去掉任何一半都会让编号行为在另一类内容上失控。
...
- AstrBot 确有 provider 侧 prompt 缓存（Anthropic 的 `cache_control: ephemeral`、
  Gemini 的 `cached_content_token_count`），属服务端前缀缓存。**历史内容变化
  会让该前缀缓存失效并重建** —— 表现为该轮首 token 延迟与输入计费略有上升，
  之后恢复正常。这是需要如实告知用户的代价，不是错误。

措辞上经历了一次修正，值得记录：

- 初版写的是「**不会破坏任何缓存**」。这句本身不算错，但偏向**打包票** ——
  读者会以为没有代价，而实际上缓存重建是真实存在的成本。
- 现改为「**可能导致对话缓存重建**」。它同时说清了「结构不会被破坏」与
  「前缀缓存会重建」两层事实，更有信息量。

教训：**「不破坏」不等于「无代价」**。写免责式结论时，要问自己
「读者会不会因此低估成本」—— 若会，就该把代价一并写出。

### 未唤醒时必须提前退出

群里发一张卡片但没 @bot 时，AstrBot 的唤醒检查会拦住 LLM 请求，bot 不回复。
但**插件原本照样做完整套重活**：抓分享页、下载十几张长图、切片几十张 —— 全部白做。

这个代价不小。实测一次群聊场景：同一条瓜条被完整抓取+切片两次
（一次是没 @bot 的裸卡片，一次是随后的引用+@bot），每轮 41 张图、8.27 MB。

修法是取 AstrBot 自己的标志位，而**不自己重建唤醒规则**：
...
`_will_wake_bot()` 读 `event.is_at_or_wake_command`：

- 由 `waking_check` 阶段在命中 @bot / 唤醒前缀 / 私聊直达时置 `True`，默认 `False`
- `process_stage/stage.py:58` 正是用它决定要不要走 LLM 链路

所以这个字段就是「本轮会不会真的请求 LLM」的权威判据，与 AstrBot 的行为天然一致，
不会因为唤醒规则变化而失效。

**取不到该属性时返回 `True`（保守放行）**：宁可多干活，也不能漏掉本该处理的分享。
兼容性优先于省资源。
